...
sealed class EventHandler<T>(val handler: (T) -> Unit) {

    abstract fun <A: Any> accepting(eventType: KClass<A>): Boolean

}

class EventTypeHandler<T: Any>(private val eventType: KClass<T>, handler: (T) -> Unit) : EventHandler<T>(handler) {
    override fun <A : Any> accepting(eventType: KClass<A>): Boolean = eventType == this.eventType
}

class AllTypesHandler<T>(handler: (T) -> Unit) : EventHandler<T>(handler) {
